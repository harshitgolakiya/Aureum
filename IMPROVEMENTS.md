...
- [x] Expand automated regression coverage and CI. The browser matrix now covers normal and reduced-motion rendering, document hierarchy, form/link labels, ARIA references and four core interaction journeys. GitHub Actions runs the complete verification suite on every `main` push and pull request. A transient homepage animation overflow exposed by the expanded checks was corrected.
